...
*/
	function loadStatsPage(){
		...
		for(var i = 2; i < table.rows.length; i++){
			var visitor = parseInt(table.rows[i].cells[3].innerHTML);
			var home = parseInt(table.rows[i].cells[2].innerHTML);
			if(visitor > home){
...
*/
	function loadPlayersPage(){
		...
		for(var i = 0; i < players.length; i++){
			selection += '<a class = "dropdown-item" href = "#" onclick = "switchPlayers(' + i + ')" >' + players[i].name + '</a>';
		}
		document.getElementById("player_selector").innerHTML = selection;
	}			
